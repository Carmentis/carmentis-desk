...
const verifiablePresentation = computedAsync(async () => {
    ...
    const sk = Ed25519PrivateSignatureKey.genFromSeed(
        new SeedEncoder().decode(seed).slice(0, 32),
    );
            ...
            hasher: async (data) => {
                const contentToHash =
                    typeof data === 'string'
                        ...
                        : data;
                const hashBytes = await window.crypto.subtle.digest(
                    'SHA-256',
                    contentToHash,
                );
...
const vct = computed(() =>
    typeof payload.value?.['vct'] === 'string'
        ? (payload.value['vct'] as string)
        : null,
);
const iss = computed(() =>
    typeof payload.value?.['iss'] === 'string'
        ? (payload.value['iss'] as string)
        : null,
);
...
watch(
    ...
    () => {
        selectedDigests.value = new Set(
            namedDisclosures.value.map((d) => d._digest),
        );
    },
    { immediate: true },
);
    ...
    >
        <div v-if="envelope" class="space-y-5">
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <span v-if="iss" class="text-xs text-gray-400">
                    · {{ iss }}
                </span>
            </div>
            ...
            <div>
                ...
                <div
                    v-if="namedDisclosures.length > 0"
                    class="border rounded-lg divide-y overflow-hidden"
                >
                    <div
                ...
                <p
                    v-if="arrayDisclosures.length > 0"
                    class="text-xs text-gray-400 mt-2"
                >
                    <i class="pi pi-info-circle mr-1"></i>
                    {{ arrayDisclosures.length }} array-element disclosure{{
                        arrayDisclosures.length !== 1 ? 's' : ''
                    }}
                    always included.
                </p>
            </div>
