...
function isNodeActive(orgId: number, nodeId: number) {
    return (
        route.name === 'node-detail' && Number(route.params.orgId) === orgId && Number(route.params.nodeId) === nodeId
    );
}
    ...
    <div v-if="wallet" class="h-full bg-gray-50 border-r border-gray-200 overflow-y-auto flex flex-col">
        <div class="flex-1 py-4 pr-4">
            ...
            >
                ...
                >
                    <i class="pi text-xs" :class="walletExpanded ? 'pi-chevron-down' : 'pi-chevron-right'"></i>
                </button>
            </div>
