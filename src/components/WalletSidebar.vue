...
const chainEndpoint = computed(
    () => wallet.value?.nodeEndpoint || 'Not connected',
);
...
function navigateToApplication(orgId: number, appId: number) {
    emit('navigate');
    router.push(
        `/wallet/${props.walletId}/organization/${orgId}/application/${appId}`,
    );
}
...
function navigateToNode(orgId: number, nodeId: number) {
    emit('navigate');
    router.push(
        `/wallet/${props.walletId}/organization/${orgId}/node/${nodeId}`,
    );
}
...
<template>
    <div
        v-if="wallet"
        class="h-full bg-gray-50 border-r border-gray-200 overflow-y-auto flex flex-col"
    >
        <div class="flex-1 py-4 pr-4">
            ...
            <div
                v-if="walletExpanded"
                class="ml-3 border-l-2 border-gray-200 pl-2 space-y-1 mb-1"
            >
                ...
                >
                    ...
                    <div
                        v-if="isOrgExpanded(org.id)"
                        class="ml-3 border-l-2 border-gray-200 pl-2 space-y-1"
                    >
                        <!-- Nodes -->
                        <div
                        ...
                        <div
                            ...
                            class="flex items-center gap-2 px-3 py-1.5 rounded-lg cursor-pointer transition-colors"
                            :class="
                                isApplicationActive(org.id, app.id)
                                    ? 'bg-blue-100 text-blue-900 font-semibold'
                                    : 'hover:bg-gray-100 text-gray-600'
                            "
                        >
