...
async function exportPublicKeyIntoFormat(
    publicSignatureKey: Ed25519PublicSignatureKey,
    format: SupportedPkFormat,
) {
    const jwk =
        await JwkSignatureKeyExporter.exportPublicKey(publicSignatureKey);
    if (format === 'jwk') {
        return jwk;
    }
...
async function approve() {
    ...
    try {
        ...
        let encoderPk: string | object = await exportPublicKeyIntoFormat(
            pk,
            pkFormat,
        );
        ...
        <Card class="w-full max-w-md">
            ...
            <template #content>
                <div class="space-y-4">
                    ...
                    >
                        <p
                            class="text-xs text-blue-500 font-semibold uppercase tracking-wide mb-1"
                        >
                            Requesting party
                        </p>
                    ...
                    <p class="text-sm text-gray-700">
                        This application is requesting you to authenticate with
                        your public key.
                    </p>
                    ...
                    <div>
                        ...
                        <p
                            class="text-xs font-mono text-surface-600 break-all bg-surface-50 rounded p-2"
                        >
                            {{ b64Challenge }}
                        </p>
                    </div>
