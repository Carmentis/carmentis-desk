...
<template>
  <div class="font-mono text-xs rounded-lg border border-surface-200 overflow-auto bg-surface-50">
    ...
    <div
      ...
      :style="{ paddingLeft: `${node.depth * 14 + 10}px`, paddingRight: '10px' }"
    >
      ...
      <template v-else>
        ...
        <!-- Key label -->
        <span v-if="node.displayKey" class="text-surface-500 mr-1 flex-shrink-0">
          {{ node.displayKey }}:
        </span>
        ...
        <template v-if="node.isExpandable">
          <span v-if="collapsed.has(node.id)" class="text-surface-400">
            <span v-if="node.type === 'array'">
              [<span class="text-primary">{{ node.childCount }}</span>]
            </span>
            <span v-else>
              {<span class="text-primary">{{ node.childCount }}</span>}
            </span>
        ...
        <template v-else-if="node.type === 'string'">
          <span v-if="node.value.length <= MAX_STRING_LENGTH || expandedStrings.has(node.id)" class="text-emerald-700 break-all">
            "{{ node.value }}"
          </span>
        ...
        <span v-else-if="node.type === 'number'" class="text-blue-600">{{ node.value }}</span>
        <span v-else-if="node.type === 'boolean'" class="text-violet-600">{{ node.value }}</span>
